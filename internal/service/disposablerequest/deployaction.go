...
func DeployAction(svcCtx *service.ServiceContext, crCtx *service.DisposableRequestCRContext) error {
	...
	pa, pollingSupported := asPollingAware(crCtx)

	// Resume an in-flight long-running operation using the persisted anchor without
	// re-firing the trigger request.
	if pollingSupported && pa.GetPolling() != nil && pa.GetPollingResponse() != nil {
		svcCtx.Logger.Debug("Resuming in-flight polling operation")
		return resumePolling(svcCtx, crCtx, pa)
	}
	...
	// Polling: when the trigger request succeeded, drive the long-running operation to
	// completion instead of finalizing immediately.
	if pollingSupported && pa.GetPolling() != nil {
		...
		return beginPolling(svcCtx, crCtx, pa, details)
	}
